...
-   ❸ Call `execute` to execute the query and return data.  
    ...
    -   `stream`: Return a `java.util.stream.Stream` backed by the JDBC cursor.
        Unlike `execute`, it does not materialize all rows as a list. The stream
    ...
    -   `count`: Based on the original data query written by users, automatically generate the count aggregate query needed for paging, execute and get total rows before paging.  
        ...
        :::info
        This is a smart and advanced operation, please see [Paging](./paging/index.md).
        :::
...
Use `stream` when the result can be processed sequentially and should not be
fully retained in memory.
...
Closing the stream closes its JDBC result set and statement, including when the
stream is only partially consumed. Always use try-with-resources in Java or
`use` in Kotlin.

The stream can contain scalar values, tuples, embeddable objects, or entities.
An entity fetcher may use associations loaded by SQL joins, but it cannot use an
association that requires a secondary select after the root rows are read. Such
post-fetch processing needs the complete root result and is therefore rejected
for streaming.

:::tip
Like `execute`, `stream` has an overload that accepts an explicit JDBC
`Connection`.
:::

### JDBC Options

The JDBC fetch size and query timeout can be configured for an individual query:
...
`fetchSize` is a driver hint for the number of rows fetched per round trip;
drivers and databases may interpret it differently. `queryTimeout` uses the
JDBC timeout in seconds.

Defaults for all statements can be set with
`setDefaultJdbcFetchSize` and `setDefaultJdbcQueryTimeout` on the SQL client
builder. A local value overrides the corresponding client default.
