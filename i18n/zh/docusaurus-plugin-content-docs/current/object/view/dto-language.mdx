...
`src/main/dto`目录下可以定义若干个dto文件。默认情况下，文件路径和名称决定文件中声明
所使用的实体。单个声明也可以通过`for`选择其他实体，因此一个文件可以描述多个实体的DTO。
有关多目标文件、import、DTO复用和片段的内容，请参考[DTO组合](./composition)。
...
对关联属性的处理是DTO语言一个非常强大的功能，存在四种用法
...
Output DTO可以配置每个关联的加载方式。配置指令需要紧挨着写在关联属性之前：
...
-   `!fetchType(SELECT | JOIN_IF_NO_CACHE | JOIN_ALWAYS)`控制引用关联的加载方式。
    `JOIN_ALWAYS`对non-null引用使用inner join，对nullable引用使用left join；DTO不会独立
    指定SQL join type。

-   `!where(...)`使用简单谓词过滤关联行。

-   `!orderBy(...)`、`!limit(limit, offset)`和`!batch(size)`用于配置集合加载，
    其中offset可以省略。

-   需要任意Jimmer DSL逻辑时，可以用`!filter(FilterType)`代替`!where`和`!orderBy`。
    递归关联还支持`!depth`和`!recursion`。

这些指令影响生成的fetcher，而不影响生成的DTO数据类，并且仅适用于output DTO。
完整语法以及自定义filter/recursion的实现方式请参阅
[Output DTO关联配置](../../query/object-fetcher/dto)。
...
甚至还可以嵌套使用`flat`函数，将多级关联转化为平坦对象
...
-   `fold`是把当前层级的多个属性重新分组为一个嵌套DTO对象。
...
这个DTO不再把`name`和`edition`暴露为顶层属性，而是通过一个嵌套属性`summary`来表达它们。
...
默认情况下，fold后的嵌套属性是非null的。如果需要可空对象，可以使用`?`：
...
此时，嵌套对象仍然会被`flat`提升，因此生成的属性名会变成`storeKey`。

`fold`支持`view`、`input`和`specification`。

-   对`input`而言，调用`toEntity()`时，fold中的属性会被重新拆回当前实体，不会额外创建一层实体结构。

-   对`specification`而言，`fold`只是DTO结构工具，用于分组查询参数，并不会改变底层查询语义。
...
DTO语言通过[复用关联DTO和片段](./composition)提供明确的复用机制。实现Java/Kotlin接口
解决的是另一个问题：让生成的DTO符合应用中已有的API契约。接口中的抽象属性必须是DTO
属性的子集，并且能由生成的DTO类型实现。
