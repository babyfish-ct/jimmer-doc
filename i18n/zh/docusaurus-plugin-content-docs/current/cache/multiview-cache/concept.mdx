...
多视图缓存由[用户自定义全局过滤器](../../query/global-filter/user-filter)导致。

:::info
只要某个实体被施加了[用户自定义全局过滤器](../../query/global-filter/user-filter)，
...
-   SubKey
    ...
    这点非常重要。比如，SubKey只可能是`{"module":"x","tenant":"a"}`，而不可能与之不同但等价的`{"tenant":"a", "module":"x"}`。
...
对于关联属性而言，缓存的多视图化必然是因其关联实体被[用户自定义全局过滤器](../../query/global-filter/user-filter)影响导致。
