...
-   The delete command supports two types of operations:
    ...
    -   `AUTO`: Does not explicitly specify whether the current operation is logical delete or physical delete
        ...
        -   If the deleted entity type has a [logical delete field](../mapping/advanced/logical-deleted/entity.mdx), perform logical delete.
    ...
    -   `PHYSICAL`: Explicitly specifies that the current operation is physical delete
        ...
        Regardless of whether the deleted entity type has a [logical delete field](../mapping/advanced/logical-deleted/entity.mdx), perform physical delete.

## Logical Delete  

Assuming the entity has a [logical delete field](../mapping/advanced/logical-deleted/entity.mdx), for example:
...
If the entity does not have a [logical delete field](../mapping/advanced/logical-deleted/entity.mdx), or the delete mode is explicitly specified as `DeleteMode.PHYSICAL`, physical delete is performed. 
...
Unlike JPA, Jimmer does not allow direct use of `@OneToMany` for association mapping. `@OneToMany` must use the `mappedBy` property. See [@OneToMany](../mapping/base/association/one-to-many.mdx) to learn more.  
...
Next, Jimmer will refer to the [@OnDissociate](../mapping/advanced/on-dissociate.mdx) annotation on the many-to-one association property `Book.store`.

<DissociateMode/>

## Dissociation Examples
1.  If the foreign key corresponding to `Book.store` is configured by the [@OnDissociate](../mapping/advanced/on-dissociate.mdx) annotation as `SET_NULL`, the following SQL is executed:
...
2.  Otherwise, first execute:
    ...
    - If the foreign key corresponding to `Book.store` is configured by [@OnDissociate](../mapping/advanced/on-dissociate.mdx) as `DELETE`, use the new delete command to delete these discarded objects. This is actually the automatic recursive execution capability of the delete command.

    - Otherwise, throw an exception.
    
The above discussed situations all require the developer to use the [@OnDissociate](../mapping/advanced/on-dissociate.mdx) annotation on the `Book.store` property. 

However, you can also choose not to use the [@OnDissociate](../mapping/advanced/on-dissociate.mdx) annotation, but dynamically specify the dissociateAction configuration for the delete command.
...
Here, calling the command's `setDissociateAction` method dynamically has the same effect as using the [@OnDissociate](../mapping/advanced/on-dissociate.mdx) annotation statically on the `Book.store` property and specifying cascade delete.
...
2.  If dissociation rules are configured dynamically for the save command and also configured statically in the entity interface through the [@OnDissociate](../mapping/advanced/on-dissociate.mdx) annotation, the dynamic configuration takes precedence. 
:::
