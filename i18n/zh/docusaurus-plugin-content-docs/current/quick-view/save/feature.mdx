...
这种绝对的灵活性，存在很多妙用，比如：讲id为100的书籍的价格修改为60，传统ORM和Jimmer做法不同。
...
:::warning
因此，强大的Jimmer数据保存能力只能在服务内部作为底层支撑，不能用动态实体作为输入参数讲此能力直接暴露出去，因为这会导致安全门户大开。

至于如何安全地暴露Jimmer的数据，请继续阅读下一篇文章：[暴露功能](./export)
:::