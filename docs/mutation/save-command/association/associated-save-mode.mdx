...
In [the previous article](../save-mode.mdx), we introduced how to control the save mode of aggregate root objects.
...
-   **saveInputs**
    ...
    Here, the `BookInput` class is not an entity type, but rather an automatically generated [Input DTO](../input-dto/index.md) type by Jimmer.
...
To demonstrate simpler SQL generation, we assume that the `targetTransferable` feature of `sqlClient` is enabled, which was previously covered, see [here](./owner.mdx#2-configuring-whether-different-parent-objects-can-snatch-for-child-objects)
...
To demonstrate simpler SQL generation, let's assume that the `targetTransferable` feature of `sqlClient` is enabled, which was previously introduced, please refer to [here](./owner.mdx#2-configuring-whether-different-parent-objects-can-snatch-for-child-objects)
