...
另外，因为save指令只需一个函数调用，采用`Repository`对其进行封装意义不大。为了简化非必要复杂行，本文不再定义`Repository`，而是让`Controller`直接使用`sqlClient`。
...
-   ❸ 如果为原实体的`id`属性配置了自动增长策略 *(比如自动变化，序列，UUID，雪花ID)*，那么在DTO语言中使用`input`修饰符会导致DTO的id属性为null。
    ...
    对于没有制定`INSERT`或`UPDATE`的保存操作而言。
...
-   提供两种可以保存书籍的角色
    ...
    -   Adminsitrator: 能新建和编辑书籍的所有信息

-   针对Adminsitrator角色的新建Api和编辑Api需要分开
...
/** 
 * 供`Adminsitrator`角色新建书籍的input, 没有id属性
 */
...
/**
 * 供`Adminsitrator`角色修改书籍的input, id属性不能为null
 */
