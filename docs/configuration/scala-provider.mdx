...
In previous documents, we have introduced how to [map enums](../mapping/advanced/enum.mdx), and how to use `@Serialized` to [map JSON](../mapping/advanced/json.mdx).
...
The [Enum Mapping](../mapping/advanced/enum.mdx) and [JSON Mapping](../mapping/advanced/json.mdx) we discussed before are actually Jimmer's built-in implementations of `ScalarProvider`.
:::
...
:::tip
The `@Serialized` annotation discussed in [JSON Mapping](../mapping/advanced/json.mdx) can annotate both the return type of properties and entity properties. 
