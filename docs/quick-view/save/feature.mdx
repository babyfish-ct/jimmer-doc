...
-   Save simple object  
    ...
    This is because in the [entity definition](../precondition.mdx#book), `Book.name` and `Book.edition` are annotated with `@org.babyfish.jimmer.sql.Key`.  
    This article is just a quick preview and does not go deep into it. Interested parties can view [Mapping Part/Advanced Mapping/Key](../../mapping/advanced/key.mdx) and [Mutation Part/Save Command](../../mutation/save-command/index.md).
    :::  
...
To safely expose Jimmer's data saving capabilities, please continue reading the next article: [Exposing Features](./export/index.md)  
:::