...
In the [previous article](./entity.mdx), we introduced that the web service does not need to define DTO types, directly returns entities, and uses `@FetchBy` annotations to restore all DTO type definitions in automatically generated client code.  
...
In the [Feature Introduction](../feature.mdx#improved-repository) article, we wrote a `BookRepository` class  
...
In the [previous article](./entity.mdx), we mentioned that Jimmer can copy the document comments in Java/Kotlin code to the client Api, whether it is OpenApi online documentation or generated TypeScript code.
...
If you choose to return DTO *(instead of directly returning entities as in [the previous article](./entity.mdx))*, you can define the following DTO code:  
