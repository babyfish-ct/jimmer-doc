...
- Save simple object  
    ...
    :::note
    Here, the id attribute of the object to be saved is not specified. Jimmer will determine whether related data exists in the database according to the `name` and `edition` attributes,  
...
- Jimmer's approach, make up a truncated object out of thin air and directly update  
    ...
    boolean matched = sqlClient.update(
        Objects.createBook(draft -> {
            ...
            // No other attributes except `id` and `price` are specified
            // So no other attributes except `price` will be affected 
        })  
...
## Note: Cannot Expose Directly to the Outside  

The ability to save data structures of arbitrary shapes is too powerful to expose directly, which will lead to huge security vulnerabilities. For example:   
...
To safely expose Jimmer's data saving capabilities, please continue reading the next article: [Export Functionality](./export)  
:::