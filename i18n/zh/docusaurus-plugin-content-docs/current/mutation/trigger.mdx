...
在`application.properties`或`application.yml`中添加一个配置即可。其名称为`jimmer.trigger-type`，其值为`BINLOG_ONLY` | `TRANSACTION_ONLY` | `BOTH`。
...
-   Maxwell
    ...
    1.  进入[jimmer-examples/env-with-cache/maxwell](https://github.com/babyfish-ct/jimmer-examples/blob/main/env-with-cache/maxwell)所对应的`git clone`后本地目录
...
-   Debezium
    ...
    1.  进入[jimmer-examples/env-with-cache/debezium](https://github.com/babyfish-ct/jimmer-examples/blob/main/env-with-cache/debezium)所对应的`git clone`后本地目录
...
4.  以`tableName`、`oldJsonNode`和`newJsonNode`调用`JSqlClient.getBinLog().accept`或`KSqlClient.binLog.accept`
