...
:::info
@`org.babyfish.jimmer.sql.Key` is used together with [Save Command](../../mutation/save-command/index.md) in mutation section.
:::

<Key/>

This article only introduces the configuration of Key. For how to use it further, please refer to [Save Command](../../mutation/save-command/index.md).

## Multi-version support

Jimmer supports [logical deletion](./logical-deleted/index.md), which hides deleted data instead of actually deleting it.
...
-   When the logical deletion flag is other type
    ...
    For the boolean logical deletion shown above, when the entity is also annotated
    with `@KeyUniqueConstraint`, Jimmer can use this conditional unique index for
    key-based upsert on dialects that can express a conflict predicate, such as
    PostgreSQL. For H2 tests, Jimmer renders equivalent `merge ... on ... and
    DELETED = false` SQL. Dialects that cannot express this predicate fall back to
    a select-based decision before insert or update.
