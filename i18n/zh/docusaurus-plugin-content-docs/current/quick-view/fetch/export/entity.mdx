...
-   使用`@org.babyfish.jimmer.client.EnableImplicitApi`修饰RestController所属工程中的任何一个类。对于Spring Boot应用而言，Appplication类就是一个不错的选择。
...
无需使用JVM生态中任何其他关于自动生成OpenAPI/Swagger的框架，只需修改`application.yml`*(或`applicaiton.properties`)*，如下
...
如果选择直接返回实体 *(而非[下一篇文章](./dto)种的返回DTO)*，且想要使用关联id，需先为实体添加@IdView属性
