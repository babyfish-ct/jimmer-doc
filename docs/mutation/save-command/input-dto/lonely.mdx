...
Functionality: Resolves security issue *(this issue is discussed in [previous doc](./problem.mdx))*. 

Advantage: No need to define Input DTOs.

Disadvantage: Does not resolve API ambiguity issue *(this issue is discussed in [previous doc](./problem.mdx))*.
