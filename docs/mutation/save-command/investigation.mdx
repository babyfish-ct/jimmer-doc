...
2.  Investigate the cause of the constraint violation
    ...
    - For most databases, the original JDBC exception information from the batch SQL can determine which record violated the constraint, so only the problematic record is investigated
    
    - For Postgres, the original JDBC exception information from the batch SQL cannot determine which record violated the constraint, so all records are investigated
...
2.  Investigate the cause of the constraint violation
    ...
    - For most databases, the original JDBC exception information thrown by the batch SQL can determine which record violated the constraint, so only the erroneous record is investigated

    - For Postgres, the original JDBC exception information thrown by the batch SQL cannot determine which record violated the constraint, so all records are investigated
