...
In the current example in the [entity definition](../../precondition.mdx#book), the more suitable example as a long association to save is the `BookStore` and its associated `books` collection.  
