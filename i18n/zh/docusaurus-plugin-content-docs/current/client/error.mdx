...
```kotlin title="UserController.kt"
@PostMapping("/user")
@Throwss(UserInfoException.IllegalUserName::class)
fun registerUser(@RequestBody input: RegisterUserInput) {
    if (...某些条件...) {
...
为了方便开发和测试，可以在`application.yml`或`appliction.properties`中配置
```
jimmer:
    error-translator:
...
:::info
请注意我们定义了6种具体的异常类型，但是Web Api只`throws`了5种，`PLATFORM_ERROR_CODE`簇中的`DATA_IS_FORZEN`从未被使用，后文会讨论。
:::
