...
-   When [implementing GraphQL with Jimmer](../../graphql/index.md), [Object Fetcher](../../query/object-fetcher/index.md) *(GraphQL and Object Fetchers are homogenous functions)* should not be used in aggregate root queries, 
...
:::caution
For collection type associations, if we use the field filter of [object fetchers](../../query/object-fetcher/index.md) to implement association-level sorting, it will cause Jimmer to ignore association cache.

To make full use of association cache while keeping the sorting of the associated object collection returned by the [object fetcher](../../query/object-fetcher/index.md) query controllable, you can specify the default sorting statically on the entity.
:::
...
-   Step 3: Convert associated object id to associated object via object cache
    ...
    Such operations have been discussed in detail in [Object Cache](./object.mdx), so they are not repeated here. This article focuses on association cache.
...
:::info
Previously in [Object Fetcher](../../query/object-fetcher/index.md), we saw that Jimmer only needs one SQL statement to query all associated objects based on a batch of current objects, but here two SQL statements are needed, please view:
...
:::caution
To use Jimmer's automatic cache invalidation, [triggers](../../mutation/trigger.mdx) need to be enabled first.
:::
...
If associated objects support [logical deletion](../../query/global-filter/logical-deleted.mdx), association caching is still supported by default. 

:::warning
However, once the logical deletion is [ignored](../../query/global-filter/logical-deleted.mdx#ignore-logical-deletion-filter) or [reversed](../../query/global-filter/logical-deleted.mdx#reverse-logical-deletion-filter), association caching will be ignored.
:::
