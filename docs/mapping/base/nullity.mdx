...
-   If the associated entity has [global filters](../../query/global-filter/index.md) applied, then for one-to-one/many-to-one associations must be nullable even if they are directly based on foreign keys, unless all of the following conditions are met:
...
If `inputNotNull` is configured for an associated property, it will be given priority consideration in [database validation](../../configuration/database-validation.mdx) to verify that the foreign key field in the database cannot be null.
