...
For IdOnly objects, Jimmer provides the [@IdView](../../mapping/advanced/view/id-view.mdx) annotation to easily flatten IdOnly objects into a field.
...
Fortunately, Jimmer provides [@ManyToManyView](../../mapping/advanced/view/many-to-many-view.mdx) to abstract a many-to-many association between the two side entities on top of this three-entity structure.
...
[@ManyToManyView](../../mapping/advanced/view/many-to-many-view.mdx) properties *(e.g. `Student.courses` in this example)* can be used in object fetchers to reduce the path length.
