...
The DTO language is a very powerful feature provided by Jimmer, and is a concept highly similar to [Object Fetcher](../../query/object-fetcher/index.md), but at an earlier stage during compilation.
...
-   Generate [Object Fetchers](../../query/object-fetcher/index.md) that match perfectly with the DTO shape definitions

Using the DTO language to quickly build DTO types is a solution tailored for Jimmer with extremely high development efficiency, much faster than using [mapstruct](./mapstruct.mdx), and is the recommended approach.

### 1.3. Application Scenarios

The application scenarios of the DTO language:

-   [Serve as return results in query APIs *(optional, not recommended, don't use unnecessarily)*](../../query/object-fetcher/dto.mdx) 

-   [Serve as input parameters in save APIs *(required)*](../../mutation/save-command/input-dto/dto-lang.mdx)

-   [Serve as input parameters in super QBE queries *(optional, recommended)*](../../query/super_qbe.mdx)
...
> For Java projects, in addition to `jimmer-apt`, it may be necessary to add an extra configuration. Please refer to [Notes](#23-notes).
:::

### 2.1. Two ways to create files

Multiple dto files can be defined under the `src/main/dto` directory. By default,
the file path and name identify the entity used by declarations in that file.
Individual declarations can also select another entity with `for`, so one file
can describe DTOs for several entities. See [DTO Composition](./composition.mdx) for
multi-target files, imports, reusable DTOs, and fragments.
...
-   [Serve as return values in query APIs *(optional, not recommended, don't use unnecessarily)*](../../query/object-fetcher/dto.mdx)
...
-   specification: Declared with the `specification` keyword. It is not closely related to DTO itself, but can be used as a query parameter to support [super QBE queries](../../query/super_qbe.mdx).
...
-   `BookSpecification` and `AnotherBookSpecification` are used as query parameters to support [super QBE queries](../../query/super_qbe.mdx). The generated Java/Kotlin types implement the `org.babyfish.jimmer.Specification<E>` interface
...
-   If the entity ID property is configured with auto-increment strategy, the id property in the input DTO is nullable.
    ...
    > 
    > Jimmer has smarter strategies in this regard, please refer to [Save Commands/Save Modes](../../mutation/save-command/save-mode.mdx). This article will not repeat the details. 
...
-   Input DTO provides comprehensive and powerful support for nullable properties.
    ...
    Please refer to [Modification/Save Command/Input DTO/Handle Null Values](../../mutation/save-command/input-dto/null-handling.mdx).
    :::

### 3.3 specification-specific functionalities

The role of `specification` is similar to `input`, used to decorate input types, but `specification` does not provide the ability to convert between entity objects, but is used as a query parameter to support [super QBE queries](../../query/super_qbe.mdx). 

:::tip
[Super QBE Query](../../query/super_qbe.mdx) is a very powerful feature of Jimmer that is not explained here. Please refer to the [relevant section](../../query/super_qbe.mdx).
:::
...
:::info
-   The `#allScalars` macro can appear anywhere among the property declarations.
...
The handling of association properties is a very powerful feature of the DTO language with four usages:
...
#### Configuring Association Loading for Output Views

An output DTO can configure how each association is fetched. Put the
configuration directives immediately before the association property:
...
BookDetailView {
    ...
    // Fetch this reference in the root SQL statement
    ...
    // Configure the secondary query for this collection
...
-   `!fetchType(SELECT | JOIN_IF_NO_CACHE | JOIN_ALWAYS)` controls how a
    reference association is loaded. `JOIN_ALWAYS` uses an inner join for a
    non-null reference and a left join for a nullable reference; the DTO does
    not choose the SQL join type independently.

-   `!where(...)` filters associated rows with a simple predicate.

-   `!orderBy(...)`, `!limit(limit, offset)` and `!batch(size)` configure
    collection loading. The offset is optional.

-   `!filter(FilterType)` replaces `!where` and `!orderBy` when arbitrary
    Jimmer DSL logic is required. Recursive associations additionally support
    `!depth` and `!recursion`.

These directives affect the generated fetcher, not the generated DTO data
class. They are available only for output DTOs. For the complete syntax and
custom filter/recursion implementations, see
[Output DTO association configurations](../../query/object-fetcher/dto.mdx#association-property-specific-configurations).
...
while of course we can choose to define [@IdView](../../mapping/advanced/view/id-view.mdx) properties on the entities and 
simply reference them in DTOs, we can't always rely on entity types having declared [@IdView](../../mapping/advanced/view/id-view.mdx) properties. 
...
This code has the exact same function as the previous example, but no longer assumes the entity type always declares [@IdView](../../mapping/advanced/view/id-view.mdx) properties, which is a better solution.
...
`fold` is the structural inverse of `flat`.

-   `flat` expands a nested object into the current DTO.

-   `fold` groups several current-level properties into a nested DTO object.
...
This DTO no longer exposes `name` and `edition` as top-level properties. Instead, it has a nested `summary` property.
...
BookFoldView view = new BookFoldView(book);
...
The printed result is roughly:

```txt
BookFoldView(
    id = 10,
    summary = BookFoldView.Summary(
        name = GraphQL in Action,
        edition = 3
    )
)
...
By default, folded properties are non-null. If you need a nullable nested object, use `?`:
...
It can also be combined with `flat`:
...
In this case, the nested object is still promoted by `flat`, so the generated property name becomes `storeKey`.

`fold` is supported by `view`, `input`, and `specification`.

-   For `input`, folded properties are unpacked back into the current entity when calling `toEntity()`. They do not create an extra entity layer.

-   For `specification`, `fold` is only a DTO-structure tool. It groups query parameters but does not change the underlying query semantics.
...
DTO language provides explicit reuse through [reusable association DTOs and
fragments](./composition.mdx). Implementing a Java/Kotlin interface serves a
different purpose: It makes a generated DTO conform to an existing application
API contract. The abstract properties in the interface must be a subset of the
DTO properties and must be implementable by the generated DTO type.
...
The following DTO language features are described in separate documents:

- [Handling Null Values in Input DTO](../../mutation/save-command/input-dto/null-handling.mdx)

- [Specification DTO, Super QBE](../../query/super_qbe.mdx)

- [DTO Composition](./composition.mdx)
