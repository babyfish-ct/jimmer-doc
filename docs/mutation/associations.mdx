...
First, the association table is hidden by the object model, with no direct corresponding entity type. This problem has been explained in [Query Association Table](../query/associations.mdx), so I won't repeat it here.

In addition, in the [Save Command](./save-command/index.md) section, we introduced the save command. It can compare the existing data structure in the database with the new data structure the user wants to save. If a relation property based on the association table has changed, the association table will be modified.
...
:::tip
When the business scenario is no longer simple addition or deletion, [Save Command](./save-command/index.md) is a better choice, as follows:
:::
...
## Insert Associations from a Query

To insert many middle-table pairs supplied by a typed query, use
[Insert and Upsert from Select](./insert-from-select.mdx). Association insertion
supports conflict suppression and returning newly inserted pairs. Java accepts
an `AssociationTable`; Kotlin provides association-property overloads.

The same Java association table can be passed to `createQuery` and
`createBaseQuery`. `createAssociationQuery` remains a deprecated query alias.
