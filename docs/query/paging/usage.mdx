...
Not only can Jimmer automatically generate `count-query`, it can also optimize `count-query`. This optimization will be discussed in [next article](./unnecessary-join.mdx).
:::
...
-   Declare abstract methods according to certain conventions, and let Jimmer automatically implement them.
    ...
    You can check [Spring Section/Spring Data Style/Abstract Methods](../../spring/repository/abstract.mdx) to learn how to implement pagination queries in this way.
    :::
...
:::caution  
To simplify the discussion, this pseudocode does not consider [reverse sorting optimization](./reverse-sorting.mdx).
:::
...
-   ❷ Based on the original template query without modification, generate the `count-query`, then execute the `count-query` to get the total number of rows before pagination.
    ...
    :::tip
    Not only can Jimmer automatically generate `count-query`, it can also automatically optimize `count-query`, please refer to [Join Optimization](./unnecessary-join.mdx).
    :::
