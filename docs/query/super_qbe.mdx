...
Super QBE is a very powerful feature that uses [DTO Language](../object/view/dto-language.mdx) to generate parameter types for complex queries and automatically implement query logic.
...
The above steps, as well as how to compile DTO files, are discussed in detail in [DTO Language #2. Create File](../object/view/dto-language.mdx#2-create-file), and will not be repeated here.
...
-   Use in Spring Data Repository
    ...
    Jimmer integrates Spring Data, so Spring Data Repository can be defined, please refer to [Spring Data Style](../spring/repository/index.md) to learn more.
...
Large searches are often built from reusable specification fragments. Generated
specifications can be combined with `and`, `or`, and `not` without exposing
their internal predicates.

<Tabs groupId="language">
<TabItem value="java" label="Java">

```java
JSpecification<Book, ?> specification = JSpecification.and(
    nameSpecification,
    JSpecification.or(priceSpecification, storeSpecification)
);

JSpecification<Book, ?> negated =
    JSpecification.not(specification);
```
...
val specification = KSpecification.and(
    nameSpecification,
    KSpecification.or(priceSpecification, storeSpecification)
)

val negated = KSpecification.not(specification)
```

</TabItem>
</Tabs>

The composition methods accept varargs or an `Iterable`. Null elements are
ignored, which is convenient when optional request parameters create optional
fragments. If no non-null specification remains, the result is null and can be
passed directly to `where`.

Each fragment remains an independent predicate group, so nested combinations
preserve the requested parentheses:

```text
name AND (price OR store)
```

Specifications for types in the same entity inheritance hierarchy can also be
composed. The result uses their closest common entity type, while each derived
specification keeps its own type restriction. This allows, for example, an `OR`
between `PersonSpecification` and `OrganizationSpecification` in a root
`Client` query. Specifications from unrelated entity hierarchies are rejected.

:::caution
Use `JSpecification` in Java or `KSpecification` in Kotlin for composition.
The common `Specification` interface is the framework-neutral parameter type
and does not expose the composition helpers.
:::
...
:::tip
Specifications are used as query parameters. To support dynamic queries, all properties are nullable by default unless explicitly specified as non-nullable using `!` *(see [DTO Language #7. Nullability](../object/view/dto-language.mdx#7-nullability))*.
:::
...
We find that the code above uses the `flat` function. The `flat` function is discussed in detail in [DTO Language #10.4-flat function](../object/view/dto-language.mdx#104-flat-function), and will not be repeated here.  
...
-   ❶ `Book.store` is a reference association that does not break [pagination safety](./dynamic-join/table-ex.mdx).

    So only [dynamic JOIN](./dynamic-join/index.md) is needed to apply the filtering condition represented by the `storeName` property of the DTO object.

-   ❷ `Book.authors` is a collection association that breaks [pagination safety](./dynamic-join/table-ex.mdx).

    So [subqueries](./sub-query.mdx) must be used instead of [dynamic JOIN](./dynamic-join/index.md) to apply the filtering condition represented by the `authorName` property of the DTO object.
...
-   Multi-property mapping can only be used in `specification` and cannot be used in other types described in [DTO Language](../object/view/dto-language.mdx).
