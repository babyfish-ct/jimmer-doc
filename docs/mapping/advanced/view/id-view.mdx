...
Except for the impact on [ObjectFetcher](../../../query/object-fetcher/index.md) to be explained below, view properties do not affect ORM and core logic at all.
:::  
...
The only difference brought by different choices is that the Jackson [visibility flag](../../../object/visibility.mdx) of original association properties and view properties are different.
...
:::info
If an entity uses whole-id [@MapsId](../maps-id.mdx), an id property like `messageId` is still the real `@Id` property, not an `@IdView`.

In that case, do not annotate it with `@IdView`. `@MapsId` already defines its relationship with the association.
:::
