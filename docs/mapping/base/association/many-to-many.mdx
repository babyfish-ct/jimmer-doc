...
If no properties are specified for `@JoinTable`, and even `@JoinTable` itself is omitted (like the previous code), Jimmer will deduce the column names corresponding to the `authors` property based on the [naming strategy](../naming-strategy.mdx).
...
If you want to define more other business fields in the join table of the many-to-many association, please refer to [ManyToManyView](../../advanced/view/many-to-many-view.mdx).
```
