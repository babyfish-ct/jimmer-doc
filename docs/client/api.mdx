...
Server: [Develop GraphQL Backend with Jimmer](../graphql/index.md) 
...
-   Server perspective is producer perspective.
    ...
    Therefore, Jimmer uses dynamic entities combined with [object fetchers](../query/object-fetcher/index.md) to eliminate DTO explosions on the server side.
...
If such business-level correlations are not generic at all, then defining [complex calculated properties](../mapping/advanced/calculated/transient.mdx) for entities is not a good choice either.
