...
This page describes the default `VersionMode.OPTIMISTIC_LOCK`. For explicit
version assignment, see [Version Mode](./version-mode.mdx). To skip an update when
a business condition fails, without an optimistic-lock exception, use
[Update Conditions](./update-where.mdx).
