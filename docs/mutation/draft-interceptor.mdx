...
Here is the English translation, keeping the original code block indentations:
...
`original` is a Jimmer dynamic object. By default, only the `id` and `key` properties are loaded and accessible. However, can we control the format of the `orginal` object to allow more properties to be accessed?

The `DraftInterceptor` interface provides another default method `dependencies()` which returns a collection of properties to indicate that in addition to the id property and key property, which other properties of the `orginal` object need to be loaded.  
