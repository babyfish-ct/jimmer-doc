...
至于格式更复杂的关联对象，读者可以自行尝试，也可以直接参考官方例子[jimmer-examples/java/save-command](https://github.com/babyfish-ct/jimmer-examples/tree/main/java/save-command)或[jimmer-examples/kotlin/save-command-kt](https://github.com/babyfish-ct/jimmer-examples/tree/main/kotlin/save-command-kt)。这些功能仍然存在，无外乎生成SQL语句更多，被隐藏的细节更繁琐而已。
:::