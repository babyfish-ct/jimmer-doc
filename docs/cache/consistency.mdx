...
:::caution
In previous articles on [object cache](./cache-type/object.mdx), [association cache](./cache-type/association.mdx), and [calculated cache](./cache-type/calculation.mdx), we have discussed in detail how Jimmer automatically deletes affected caches when data is modified.
...
Whether it is the fully automatic invalidation of [object cache](./cache-type/object.mdx) and [association cache](./cache-type/association.mdx), or the user assisted invalidation of [calculated cache](./cache-type/calculation.mdx), the cache consistency of Jimmer is driven by [triggers](../mutation/trigger.mdx).
