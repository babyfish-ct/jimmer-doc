...
1.  Static language ORM represented by JPA
    ...
    -   Advantages: Convenient, code safety *(Based on a strongly typed language, most of the code is safe. If combined with QueryDSL, all code can be guaranteed to be safe)*
...
2.  Dynamic language ORM represented by ActiveRecord *(Ruby)*
    ...
    -   Advantages: For dynamic language ORMs, you only need to combine the flexibility of dynamic language object structures with ORM implementation to achieve both convenience and flexibility.

    -   Disadvantages: It is well known that dynamic languages lack maintainability and are not conducive to collaborative development by multiple people, despite being both convenient and flexible.
...
3.  Lightweight SQL Builder/Mapper represented by MyBatis
    ...
    -   Advantages: Direct writing of SQL, arbitrary and flexible; it is a strongly typed framework, with code safety *(The MyBatis ecosystem also has strongly typed SQL DSL extensions that can solve the code insecurity caused by native SQL strings)*

    -   Disadvantages: Severe lack of convenience, and extremely high redundant labor.
    ...
    :::info
    Finally, the most boastful point of this camp is: "Writing SQL directly will bring more direct control, and this direct control is superior to any ORM." In the long-term technical stagnation in this field, many developers firmly believe this.
...
2.  If you directly access a non-existent property using Java/Kotlin code, it will cause an exception. <ViewMore buttonText="See a quick demonstration"><Unloaded/></ViewMore>
    ...
    -   Or you implicitly map properties *(e.g., using [BeanUilts](https://docs.spring.io/spring-framework/docs/current/javadoc-api/org/springframework/beans/BeanUtils.html) technology)*, which introduces new safety issues that cannot be detected at compile-time.
...
4.  Recursively save tree structures <ViewMore buttonText="View quick demo" title="Recursively Save Tree Structures"><SaveRecursiveTree/></ViewMore>
...
This more convenient feature is called `Super QBE`, but due to space limitations, it will not be discussed in this article.
:::
...
|[Dynamic Table Join](../query/dynamic-table)|-|
...
|[Super QBE](../query/super_qbe)|Not shown in this article|
