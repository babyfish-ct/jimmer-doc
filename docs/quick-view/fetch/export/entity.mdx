...
-   Use `@org.babyfish.jimmer.client.EnableImplicitApi` to decorate any class in the project where the RestController belongs. For Spring Boot applications, the Appplication class is a good choice.  
...
If the associated object has only the `id` attribute, the associated id is better than the associated object. For example:  

-   Using the associated object will lead to a large number of objects with only the id attribute, making the results slightly redundant:  
...
If you choose to directly return entities *(rather than the DTO returns in the [next article](./dto))*, and want to use associated ids, you need to add @IdView attributes to the entities first:  
...
-   The `storeId` attribute is not a brand new attribute. It is just a view of the `store` attribute, getting the `id` attribute of the associated object represented by the `store` attribute *(or null)*. `storeId` and `store` share the same data.  

-   The `authorIds` attribute is not a brand new attribute either. It is just a view of the `authors` attribute, getting a list of `id` attributes of all associated objects represented by the `authors` attribute. `authorIds` and `authors` share the same data.  
