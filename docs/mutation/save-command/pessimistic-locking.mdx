...
In-transaction triggers are a type of [trigger](../trigger.mdx) supported by Jimmer. Jimmer always queries the old data before modification through select operations to simulate the trigger effect.
