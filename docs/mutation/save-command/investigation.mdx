...
2.  Investigate the cause of the constraint violation
    ...
    -   Scenario 2
        ...
        > Postgres and MySQL with batch capabilities enabled (see [MySQL Issues](./mysql.mdx)) both fall into this scenario
...
:::info
In the [Mapping/Advanced Mapping/Key](../../mapping/advanced/key.mdx) article, we introduced that entities can be configured with multiple unique constraints *(or unique indexes)* *(in fact, this documentation has not been modified yet)*.
...
In [Mapping/Basic Mapping/Real and Fake Foreign Keys](../../mapping/base/foreignkey.mdx), we introduced that Jimmer supports both real and fake foreign keys.
...
-   Translate `SaveException.IllegalTargetId` exception
    ...
    ```java
    @Component
    public class IllegalTargetIdExceptionTranslator implements ExceptionTranslator<SaveException.IllegalTargetId> {
