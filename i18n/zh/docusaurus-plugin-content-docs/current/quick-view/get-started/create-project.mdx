...
如果你使用的构建工具是Gradle，也可以使用社区提供的第三方Gradle插件，自动识别项目类型，自动添加依赖。
...
<TabItem value="gradle_plugin" label="Gradle插件">
...
plugins {
    ...
    // 如果你的项目使用 Kotlin 语言作为主要开发语言，为使本插件正确生效，必须确保你的项目应用了 KSP 插件
    // KSP插件的作用详见 “初识Jimmer - APT/KSP”
...
<TabItem value="gradle_plugin_kts" label="Gradle插件(kts)">
