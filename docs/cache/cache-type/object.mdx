...
-   2nd SQL: Fetch many-to-one association `Book.store`
    ...
    Fortunately, the associated attribute `Book.store` is based on a real foreign key. The foreign key itself represents the id of the parent object. There is no need to obtain the associated object id through additional queries or [association cache](./association.mdx).
    ...
    ```
    :::info
    We only need the `ID` and `NAME` fields, but here all fields are queried, which is different from what was discussed earlier in [Object Fetcher](../../query/object-fetcher/index.md). Please view:
