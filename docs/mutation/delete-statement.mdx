...
:::note 
If JOIN is used in a delete statement, Jimmer will translate it into `select` + `delete`. First use the `select` statement with `join` clause to query the ids of the data to be deleted, then use [Delete Command](./delete-command.mdx) to delete the data.
