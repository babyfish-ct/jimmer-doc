...
`forbidUpdate()` selects no entity-property update assignments; it does not
mean `INSERT_IF_ABSENT`, because a self-assignment can still be required by
save semantics. Check [isAccepted](./returning.mdx#acceptance) instead of deriving
acceptance from the affected-row count.
