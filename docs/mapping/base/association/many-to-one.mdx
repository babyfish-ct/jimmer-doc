...
Here `@JoinColumn` is not used together to explicitly specify the foreign key column name. Jimmer will deduce the column name corresponding to the `store` property based on the [naming strategy](../naming-strategy.mdx).

If the default [naming strategy](../naming-strategy.mdx) is not overridden by the user, the foreign key column name of the `store` property is `STORE_ID`. So the previous code is equivalent to:
...
Here, no properties are specified for `@JoinTable`. Jimmer will deduce the column names corresponding to the `store` property based on the [naming strategy](../naming-strategy.mdx).
