...
-   删除指令支持两种操作
    ...
    -   `AUTO`: 不明确指定当前操作是逻辑删除还是物理删除
        ...
        -   如果被删除的实体类型具备[逻辑删除字段](../mapping/advanced/logical-deleted/entity)，则执行逻辑删除
    ...
    -   `PHYSICAL`: 明确指定当前操作是物理删除
        ...
        无论被删除的实体类型是否具备[逻辑删除字段](../mapping/advanced/logical-deleted/entity)，都执行物理删除

## 逻辑删除

假设实体具备[逻辑删除字段](../mapping/advanced/logical-deleted/entity)，例如
...
如果实体不具备[逻辑删除字段](../mapping/advanced/logical-deleted/entity)，或删除模式被强行指定为`DeleteMode.PHYSICAL`，则执行物理删除。
