...
-   ❶
    ...
    - Java: The subquery is based on the global variable `TableExes.AUTHOR_TABLE_EX` and is aliased as `author` for convenience.
...
public class App {
    ...
    public static void main(String[] args) {
        ...
        for (Tuple3<Book, Integer, Integer> tuple : tuples) {
            System.out.printf(
                "Book object: %s, Global rank: %d, Local rank: %d\n",
                tuple.get_1(),
                tuple.get_2(),
                tuple.get_3()
            );
