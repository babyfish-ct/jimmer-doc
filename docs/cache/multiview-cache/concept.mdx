...
Multi-view cache is caused by [user defined global filters](../../query/global-filter/user-filter).

:::info
As long as an entity is applied with a [user defined global filter](../../query/global-filter/user-filter), 
...
For association properties, the necessity of multi-view cache must be caused by the associated entity being affected by [user defined global filters](../../query/global-filter/user-filter).
