...
Through the content related to [Association Mapping](../base/association/index.md), it is not difficult to find that all associations are based on `@JoinColumn` and `@JoinTable` *(may be omitted)*.
...
:::caution
ORM automatically generates SQL *(which is very important for dynamic queries, especially for ORMs like Jimmer that first proposed the [Dynamic Join](../../query/dynamic-join/index.md) concept)*, and users cannot interfere with the aliases of tables in the final SQL.
...
-   Cannot be used as [Remote Association](./remote.mdx)

-   Cannot participate in [Dynamic Join Optimization](../../query/dynamic-join/optimization.mdx) 

-   Cannot use [Association Cache](../../cache/cache-type/association.mdx)

-   Cannot be saved as part of data structure by [Save Command](../../mutation/save-command/index.md). In fact, if the data structure to be saved contains non-structural associations, it will cause exceptions.
:::
