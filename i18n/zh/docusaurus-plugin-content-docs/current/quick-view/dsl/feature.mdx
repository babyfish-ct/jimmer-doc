...
为了解决原生SQL抽象程度低的问题。Jimmer DSL支持了动态谓词、动态表连接和隐式子查询，从而让任意复杂的动态查询可以被轻松实现。
...
-   ❶ 如果`name`既然非null也非empty string，则添加SQL条件`table.name ilike name`

-   ❷ 存在如下四种情况

    -   如果`minPrice`和`maxPrice`都非null，则添加SQL条件`table.price between minPrice and maxPrice`。

    -   如果仅`minPrice`非null，则添加SQL条件`name >= minPrice`

    -   如果仅`maxPrice`非null，则添加SQL条件`name <= minPrice`

    -   如果`minPrice`和`maxPrice`都为null，则不提你啊任何添加SQL条件
...
前面的例子中，我们的动态SQL条件都是正对当前实体 *(Book)* 的。接下来我们通过引用关联 *(一对一活多对一)* 来为关联对象添加过滤条件
...
:::tip
Java代码中的路径`table.store()`或Kotlin代码中的路径`table.name`叫做动态表连接，表示如下SQL逻辑
```sql
from BOOK b
/* highlight-next-line */
...
这会导致❶和❷两处的`ilikeIf`/`ilike?`都生效，进一步导致两处的表连接`table.store()`/`table.store`都生效。即，表连接创建和使用了多次。
...
注意：这里所讨论的是隐式子查询，不是普通子查询 *(Jimmer也支持普通子查询)*。这里所讨论的规则对普通子查询不适用，普通子查询不在快速预览的范围内。
:::
...
-   第一个隐式子查询将会被忽略
    ...
    -   ③处的`or`为null将会导致隐式子查询没有任何SQL条件，这时创建子查询毫无意义，最终④处得到的子查询为null

    -   ④处得到的子查询为null，将会导致⑤处的where毫无意义，此操作将会被忽略。
