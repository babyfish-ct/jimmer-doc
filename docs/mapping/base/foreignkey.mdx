...
Through the discussion of [Association Mapping](./association/index.md), we know that association mapping requires a lot of foreign keys.
...
Therefore, the code is no different from the sample code in [Association Mapping](./association/index.md), so there is no need for sample code here.
