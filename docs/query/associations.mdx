...
:::caution
The association pseudo entity `Association<S, T>` is simple and special, does not support or need [object fetchers](./object-fetcher/index.md).  

However, its association properties `source` and `target` can support [object fetchers](./object-fetcher/index.md), such as:
...
But this is not the case. Due to optimizations like [phantom joins](./dynamic-join/optimization.mdx#phantom-joins) and [half joins](./dynamic-join/optimization.mdx#half-joins), whether association table queries are used or not, similar functionality and performance can be achieved. Using association table queries is completely up to user preference.
...
-   ❷ `association.source` is a [phantom join](./dynamic-join/optimization.mdx#phantom-joins) that does not generate a SQL join.
...
❶  `author.books` is a [half join](./dynamic-join/optimization.mdx#half-joins) that only generates SQL JOIN from `AUTHOR` to the `BOOK_AUTHOR_MAPPING` table, without further join to the `BOOK` table.
