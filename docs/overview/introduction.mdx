...
Jimmer's DSL is not a mechanical translation of SQL. Its original `dynamic table join` fills the gap in the JVM ecosystem, allowing even the most complex dynamic queries to be easily written.

:::note
The lack of `dynamic table join` is an gap in the JVM ecosystem, and also the fuse for the Jimmer project to be launched. 
:::
...
<Communication/>&nbsp;<Button href="../client" size="small">Learn More</Button>
