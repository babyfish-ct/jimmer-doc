...
-   第一列或第二列中的 ✩ 表示当前配置方法支持下一节即将讨论的多版本数据。
    ...
    :::info
    支持逻辑删除却不考虑多版本数据的问题，是一种成熟不高的考虑，因此建议使用支持多版本的逻辑删除配置。
    :::

-   `当前时钟毫秒数`，默认行为是`System.currentMillis()`，即使默认的`org.babyfish.jimmer.sql.meta.LogicalDeletedLongGenerator`的行为。
...
-   `随机UUD`，默认行为是`UUID.randomUUID()`，即使默认的`org.babyfish.jimmer.sql.meta.LogicalDeletedUUIDGenerator`的行为。
