...
`original`是Jimmer动态对象，默认情况下，只有`id`和`key`属性是已加载和可访问的。然而，是否能够控制`orginal`对象的格式让跟多的属性可以被访问呢？

`DraftInterceptor`接口提供了另外一个default方法`dependencies`，返回一个属性集合，以表示除了id属性和key属性外，`orginal`对象还有那些属性需要被加载。
...
上文中，我们定义的类`BaseEntityDraftInterceptor`被`@Compnoent`修饰，很明显这是一个Spring托管对象。
