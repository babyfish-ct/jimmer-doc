...
Jimmer provides [DTO language](../../object/view/dto-language.mdx). 
...
:::caution
Note that if the data of a certain shape queried by the server is not for its own use, but to be directly returned as the HTTP request return value, it is more recommended to directly return the dynamic entity object and use the scheme in [Generate Client Code](../../client/index.mdx) to automatically generate client code with high development experience.
:::

## Define DTO shape

This article focuses on how to query static DTO types, not a systematic introduction to the DTO language. Please refer to [Object/DTO Conversion/DTO Language](../../object/view/dto-language.mdx) for a complete DTO language.
...
2.  Under `src/main/dto`, create subdirectories `com/yourcompany/yourproject/model` according to the package path where the entity is located
...
-   Use inherited methods from `JRepository/KRepository`
...
:::tip
Related details have been discussed in detail in [Object Fetcher/DTO Query](../../query/object-fetcher/dto.mdx). This article will not repeat.
