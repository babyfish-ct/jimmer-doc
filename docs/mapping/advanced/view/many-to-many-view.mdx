...
In [Basic Mapping/Associative Mapping](../../base/association/index.md), we learned about the classic associative mappings in ORM, including one-to-one, many-to-one, one-to-many and many-to-many.
...
-   When constructing entity objects, only the original property can be set, not the view property.
    ...
    This is different from [@IdView](./id-view.mdx). For [@IdView](./id-view.mdx), both the original property and the view property can be set.

-   Whether it is the original property or the view property, the preprocessor will generate code to match them, so both can be used in [object fetchers](../../../query/object-fetcher/index.md) and strongly typed SQL DSL.
