...
Some Jimmer users provide Intellij plugins for the DTO language. For details, see https://github.com/ClearPlume/jimmer-dto
...
1.  For any Java project that needs to use the DTO language, create a subdirectory `dto` under its `src/main` directory. That is, `src/main/dto` is where DTO files are stored.   
...
-   ❶ ❹: Java `<V extends View<Book>>` or Kotlin `<V: View<Book>>` defines a generic parameter `V` representing any Output DTO type derived from `Book`.  
    
    For example: `SimpleBookView` and `ComplexBookView` generated above implement the `View<Book>` interface.  

-   ❷ ❺: Use the type of any DTO derived from `Book` as a parameter.  
...
:::info
The overall document comment of the [DTO Language](../../../object/view/dto-language.mdx) has higher priority.   

That is, the document comments in the DTO language can override the document comments of the original entity, which Jimmer automatically generates OpenApi documentation or TypeScript code.  
:::
