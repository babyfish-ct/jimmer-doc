...
In [last doc](./props.mdx) we introduced scalar property fetching. In this doc we discuss association fetching.
...
2.  After field filters are used, [association cache](../../cache/cache-type/association.mdx) for that field will be ignored. 

    To use the association cache, global filters supporting [multi-view cache](../../cache/multiview-cache/index.md) can be used.
...
-   JOIN_IF_NO_CACHE:
    ...
    -   If the association property to be fetched has enabled [association cache](../../cache/cache-type/association.mdx), equivalent to `SELECT`
