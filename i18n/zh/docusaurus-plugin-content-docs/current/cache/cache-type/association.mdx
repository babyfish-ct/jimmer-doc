...
-   在[用Jimmer实现GraphQL](../../graphql)时，本不应该在聚合根查询中使用[对象抓取器](../../query/object-fetcher) *(GraphQL和对象抓取器是同质的功能)*，
    但在聚合根查询中错误地使用了对象抓取器得到没有外键的对象。然而，GraphQL请求体中包含多对一关联。
:::
...
:::info
之前在[对象抓取器](../../query/object-fetcher)中，我们看到Jimmer只需要一句SQL就可以根据一批当前对象查询所有关联对象，而这里却需要两句，请参见：
...
如果关联对象支持[逻辑删除](../../query/global-filter/logical-deleted)，默认情况下，关联缓存仍然被支持。

:::warning
然而，一旦逻辑删除过滤器被[忽略](../../query/global-filter/logical-deleted#忽略逻辑删除过滤器)或[反转](../../query/global-filter/logical-deleted#反转逻辑删除过滤器)，关联缓存会被忽略。
:::