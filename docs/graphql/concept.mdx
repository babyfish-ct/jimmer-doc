...
In the [Spring section](../spring/index.md), we discussed how to implement REST services with Jimmer and automatically generate client code like TypeScript. 
...
Therefore, it is not possible to expose functionality like [recursive queries in object fetchers](../query/object-fetcher/recursive.mdx) through GraphQL. This is a functionality sacrifice that must currently be accepted when using GraphQL.
:::
...
This has already been discussed in detail in the [Generate Code](../quick-view/get-started/generate-code.mdx) article and will not be repeated here.
:::
...
:::caution
In the current version, there is a limitation that when providing GraphQL services, [simple calculated properties based on SQL](../mapping/advanced/calculated/formula.mdx#2-sql-based-calculation) should NOT be used.
:::
