...
-   Kotlin: `createQuery`自动创建表对象，lamba中的自动变量`table`的类型被自动识别为`Table`而非`TableEx`
:::
...
-   Kotlin: `wildSubQuery` *(或`subQuery`)* 自动创建表对象，lamba中的自动变量`table`的类型被自动识别为`TableEx`而非`Table`
:::
