...
-   The internal base-query returns two columns *(the first column is a table object, the second column is a simple expression)*. The external query uses `get_1()`/`_1` and `get_2()`/`_2` to use them.
    ...
    > row shapes, use a [typed tuple](./typed-tuple).
...
hard to read. [`@TypedTuple`](./typed-tuple) lets one declaration serve both as
...
[Typed Tuple](./typed-tuple) for Java and Kotlin declarations and complete
examples.
