...
-   For true foreign keys defined in the entity model (see [True and False Foreign Keys](../mapping/base/foreignkey.mdx)), verify that the database has exactly matching foreign key constraints.
:::
...
-   Entity types must correspond to a table in the database, whether the table name is explicitly configured by the developer via `@Table` or automatically determined by the [naming strategy](../mapping/base/naming-strategy.mdx). 

-   Association properties based on join tables must correspond to a real join table in the database, whether the table name is explicitly configured by the developer via `@JoinTable` or automatically determined by the [naming strategy](../mapping/base/naming-strategy.mdx).

-   The ID whose auto growth strategy is `SEQUENCE` must correspond to a sequence in the database, whether the sequence name is explicitly configured by the developer via `@GeneratedValue` or automatically determined by the [naming strategy](../mapping/base/naming-strategy.mdx) .
