...
3.  **Most importantly, unfriendly to pagination queries**.
   ...
   :::tip
   Note that Jimmer's other feature [Object Fetcher](../object-fetcher/index.md) does not use table join to fetch collection associations, so it does not have this problem. Feel free to use it.
   :::
...
:::tip
For collection associations, JOIN is actually not recommended. [Implicit subqueries](../implicit-subquery.mdx) are more recommended. 
