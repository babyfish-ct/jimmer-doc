...
[base query](../query/base-query.mdx). This is useful for importing query results,
...
can be queried independently. See [Select an Entity Table Directly](../query/base-query.mdx#select-an-entity-table-directly)
...
that query. A [typed tuple](../query/typed-tuple.mdx) gives its columns meaningful names.
...
in the example above. See [Version Mode](./save-command/version-mode.mdx) for the
...
a [typed-tuple mapper](../query/typed-tuple.mdx). It returns stored values after the
...
Both paths preserve defaults, [transaction events](./trigger.mdx), and cache
...
For object or graph input, use a [save command](./save-command/index.md) instead. Its
[assignment expressions](./save-command/assignment.mdx), [update conditions](./save-command/update-where.mdx),
and [upsert mask](./save-command/upsert-mask.mdx) provide the corresponding controls.
