...
:::caution
Unlike previous discussions where cache can be categorized into [object cache](../cache-type/object.mdx), [association cache](../cache-type/association.mdx) and [calculated cache](../cache-type/calculation.mdx), 
multi-view cache is not adding a new category to the existing taxonomy, but rather another dimension of characteristics.
:::

Except for [object cache](../cache-type/object.mdx), both [association cache](../cache-type/association.mdx) and [calculated cache](../cache-type/calculation.mdx) can be combined with multi-view cache.
...
Multi-view cache is caused by [user defined global filters](../../query/global-filter/user-filter.mdx).

:::info
As long as an entity is applied with a [user defined global filter](../../query/global-filter/user-filter.mdx), 
...
Don't worry. Jimmer will tell developers [why cache is abandoned](./abandoned-callback.mdx).
:::
...
Here we list examples of [object cache](../cache-type/object.mdx), [association cache](../cache-type/association.mdx) and [calculated cache](../cache-type/calculation.mdx).
...
<table>
    ...
    <tbody>
        ...
        <tr>
            <td>&#123;"module":"x","tenant":"a"&#125;</td>
            <td>[5,3,9]</td>
        </tr>
        <tr>
            <td>&#123;"module":"x","tenant":"b"&#125;</td>
            <td>[6,2,8]</td>
        </tr>
        <tr>
            <td>&#123;"module":"y"&#125;</td>
        ...
        <tr>
            <td>&#123;"module":"y","tenant":"a"&#125;</td>
            <td>[1,7]</td>
        </tr>
        <tr>
            <td>&#123;"module":"y","tenant":"b"&#125;</td>
            <td>[4]</td>
        </tr>
        <tr>
...
For association properties, the necessity of multi-view cache must be caused by the associated entity being affected by [user defined global filters](../../query/global-filter/user-filter.mdx).
...
Implementing [complex calculated properties](../../mapping/advanced/calculated/transient.mdx) requires implementing the `TransientResolver/KTransientResolver` interface.
...
-   If `Ref` itself is null, it means getting sub key is difficult and multi-view cache cannot be applied to the calculated property.
    ...
    :::info
    In this case, Jimmer will tell developers [why cache is abandoned](./abandoned-callback.mdx).
    :::
