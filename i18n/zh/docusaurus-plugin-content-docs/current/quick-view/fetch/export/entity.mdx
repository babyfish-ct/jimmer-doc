...
如果选择直接返回实体 *(而非[下一篇文章](./dto)种的返回DTO)*，如果要使用关联id，可以先为实体添加@IdView属性
...
-   `storeId`属性并非全新属性，而是`store`属性的视图，获取`store`属性所表示的关联对象的`id`属性。`storeId`和`store`共享相同的数据。

-   `authorIds`属性并非全新属性，而是`authors`属性的，获取`authors`属性所表示的所有关联对象的`id`属性。`authorIds`和`authors`共享相同的数据。

这是，如此编写REST Controll即可
...
public class BookController implements Fetchers {
    ...
    /**
     * Shallow Book DTO which can access properties of `Book` itself and all associated ids
     */
    private static final Fetcher<Book> SIMPLE_BOOK =
            SHALLOW_BOOK
                    .allScalarFields()
