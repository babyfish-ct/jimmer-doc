...
Previous documents have discussed in detail the [dynamic](./dynamic.mdx) and immutable properties of Jimmer entities. This introduces a new feature: visibility.
...
-   Java/Kotlin-based calculated properties *([review details](../mapping/advanced/calculated/formula.mdx#1-javakotlin-based-calculation))*
...
-   @IdView view properties *([review details](../mapping/advanced/view/id-view.mdx))*
...
-   @ManyToManyView view properties *([review details](../mapping/advanced/view/many-to-many-view.mdx))*
...
When using an [object fetcher](../query/object-fetcher/index.md) to fetch a view property, the internal logic will translate it into fetching the original property, for example:
...
Unlike [dynamic](./dynamic.mdx) where a property can be loaded or unloaded, visibility is an orthogonal feature, completely unrelated to dynamism.

Visibility only affects [Jackson serialization](./jackson.mdx) of objects *(including their own `toString` behavior)*, deciding whether a property is serialized. Other than that, it does not impact any other behavior of the object.
...
See [tool methods](./tool.mdx) for how to control visibility of Jimmer object properties yourself
:::
