...
Jimmer Input DTOs are introduced in great detail in [Save Command/Input DTO](../mutation/save-command/input-dto/index.md), which will not be repeated here.

Jimmer provides two ways to define Input DTOs:

-   [Automatically generate Input DTOs using the DTO language](../object/view/dto-language.mdx) 
-   [Manually define Input DTOs based on MapStruct](../object/view/mapstruct.mdx)
...
-   ❸ [Save command](../mutation/save-command/index.md) to save arbitrary data structures in one line

    Here `bookRepository.save(input)` is actually shorthand for `bookRepository.save(input.toEntity())`.

    :::tip
    No matter how simple or relatively complex the data structure defined by the `BookInput` type, it can be saved in one line. This is the core value of the [save command](../mutation/save-command/index.md) feature.
    :::
```